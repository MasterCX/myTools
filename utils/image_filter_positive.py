...
path_origin = "D:\\img\\zhizhen\\0923\\position2"
path_target = "D:\\img\\zhizhen\\0923\\position2-sub"
...
def get_boundingRect(cnt):
    ...
    box = cv2.boxPoints(
        ((center_x, center_y), (width, height), angle))  # 计算最小面积矩形的坐标
    ...
    for root, dirs, files in os.walk(path_origin):
        for file in files:
            ...
            if suffix in ["jpg", "jpeg", "png"]:
                ...
                if len(contours) >= 1:
                    box, angle, width, height, center_x, center_y, area = get_boundingRect(
                        contours[contour_largest])
                    # print(box)
                    ...
                    save_process = multiprocessing.Process(
                        target=save_image, args=(image_cv2, file_name, image_path))
                    save_process.start()
